...
def get_optim(model, args):
    if args.model_type == 'clam_adgraph':
        ...
        for name, param in model.named_parameters():
            ...
            elif 'head_score' in name:
                head_score_params.append(param)
            else:
                theta_params.append(param)

        if args.opt == 'adam':
            optimizer = torch.optim.Adam([{'params':other_params, 'lr':args.lr, 'weight_decay':args.reg},
                                          {'params':theta_params, 'lr':args.lrTheta, 'weight_decay':args.reg},
                                          {'params':head_score_params, 'lr':args.lrTheta, 'weight_decay':args.reg}])
        elif args.opt == 'sgd':
            optimizer = torch.optim.SGD([{'params':other_params, 'lr':args.lr, 'weight_decay':args.reg},
                                         {'params':theta_params, 'lr':args.lrTheta, 'weight_decay':args.reg},
                                         {'params':head_score_params, 'lr':args.lrTheta, 'weight_decay':args.reg}])
        else:
